...
#	$self->{RPC} = AliEn::RPC->new or $self->info("Error creating the RPC module") and return;
...
"And the job was executed with $d. You can find the output of the job in $agent->{WORKDIR} (on your local machine)"
  ...
  if (!$done) {
    $self->{LOGGER}->error("CE", "=====================================================");
    $self->{LOGGER}->error("CE", "Cannot enter your job, The Job Manager did not reply!");
    $self->{LOGGER}->error("CE", "=====================================================");
    return;
	}
  my $jobId = $done->result;

#	$self->{RPC} or $self->{RPC} = AliEn::RPC->new;
#
...
sub getNumberFreeSlots {
	...
	#return -1000 if $queued==$BAD_CE_QUERY_ID; 
	...
	$self->info("Returning $free free slots, with $running running jobs");

	if ($self->{MONITOR}) {
		$self->{MONITOR}->sendParams(
			{    'jobAgents_queued'  => $queued,
				#'jobAgents_running' => ($running - $queued),
				'jobAgents_running' => $running,
...
sub f_ps_traceAgent {
  my $self = shift;
  my $done=  $self->{SOAP}->CallSOAP( "Manager/JobInfo", "getTraceAgent",  @_);
  $done or return;
  $self->info("The call was successful");
  my $d=$done->result;
  my @info;
  eval {
   ($d and $d->{rcmessages}) and $self->info(join("", @{$d->{rcmessages}}));
   $d and $d->{results} and @info=@{$d->{results}};
  };
  return @info;
}
sub f_ps_trace {
...
#    use Data::Dumper;
#    print Dumper($_);
#    push @jobs, join ("###", $_->{queueId}, $_->{status}, $_->{name}, $_->{execHost}, "$_->{user}\@$_->{submitHost}",
...
#  (@jobs) or (push @jobs, "\n");
  ...
  $self->info( "ps done with ". $#{$rresult} ."  entries");
...
"\t         \t:  INSERTING,WAITING,OVER_WAITING,ASSIGEND,QUEUED,STARTED,RUNNING,DONE,ERROR_%[A,S,I,IB,E,R,V,VN,VT]\n";
  ...
  if ($args[0] eq "-jdl") {
    $self->info("Calling in fact the normal ps_jdl");
    shift @args;
    my $d= $self->f_ps_jdl(@args);
    return ({jdl=>$d});
  }
  if ($args[0] eq "-trace" ) {
    shift @args;
    my $d=$self->f_ps_trace(@args);
    return @$d;
  }
...
#"status='RUNNING' or status='WAITING' or status='OVER_WAITING' or status='ASSIGNED' or status='QUEUED' or status='INSERTING' or status='SPLIT' or status='SPLITTING' or status='STARTED' or status='SAVING'";
   my $sqlstatus =
"statusId=10 or statusId=5 or statusId=21 or statusId=6 or statusId=1 or statusId=3 or statusId=2 or statusId=7 or statusId=11";
  my $sqlusers      = "  ";
  my $sqlsites      = "  ";
  my $sqlnodes      = "  ";
  my $sqlmasterjobs = "  ";
...
#      $sqlstatus .= "$or status='RUNNING' or status='STARTED' or status ='SAVING'";
      $sqlstatus .= "$or statusId=10 or statusId=7 or statusId=11 " ;
      $or = "or";
    }

    if ($flags =~ /q/) {
#      $sqlstatus .= "$or status='QUEUED' or status='ASSIGNED'";
      $sqlstatus .= "$or statusId=6 ";
      $or = "or";
    }

    if ($flags =~ /f/) {
      $sqlstatus .= "$or (statusId<=-1 and statusId>=-13 ) or statusId=-16 or statusId=-17 or statusId=-18 or statusId=-19 ";
      $or = "or";
    }

    if ($flags =~ /d/) {
      $sqlstatus .= "$or statusId=15 ";
      $or = "or";
    }

    if ($flags =~ /t/) {
      $sqlstatus .= "$or statusId=15 or (statusId<=-1 and statusId>=-11 ) or statusId=-16 or statusId=-17 or statusId=-18 or statusId=-19 ";
    }

    if ($flags =~ /s/) {
      $sqlstatus .=
"$or statusId=1 or statusId=21 or statusId=5 or statusId=7 ";
    }
    ...
    foreach (@allstatus) {
      my ($st) = $self->{TASK_DB}->queryValue("select statusId from QUEUE_STATUS where status like ?",
                                                         undef, {bind_values=>[$_]});
      $st and $sqlstatus .= "statusId=$st or";
    }
    chop $sqlstatus;
    chop $sqlstatus;
  }
...
#  foreach (@allusers) {
#  	my ($st) = $self->{TASK_DB}->queryValue("select userId from QUEUE_USER where user like ?",
#                                                         undef, {bind_values=>[$_]});
#    $st and $sqlusers .= " userId=$st or";
#  }
  
  foreach (@allusers) {
    $sqlusers .= " user like '$_' or";
  }
...
#  foreach (@allsites) {
#    my ($st) = $self->{TASK_DB}->queryValue("select siteId from SITEQUEUES where LOWER(sitename) like LOWER(?)",
#                                                     undef, {bind_values=>[$_]});
#    $st and $sqlsites .= "siteId=$st or";
#  }
...
#  foreach (@allnodes) {
#  	my ($st) = $self->{TASK_DB}->queryValue("select hostId from QUEUE_HOST where host like ?",
#                                                 undef, {bind_values=>[$_]});
#    $st and $sqlnodes .= "nodeId=$st or";
#  }

  foreach (@allnodes) {
    $sqlnodes .= "host like '$_' or";
  }
  ...
  $sqlusers      eq "" and $sqlusers      = "1" 
                       or  $sqlJoin .= " join QUEUE_USER using(userId) ";
  $sqlsites      eq "" and $sqlsites      = "1"
                       or  $sqlJoin .= " join SITEQUEUES using(siteId) ";
  $sqlnodes      eq "" and $sqlnodes      = "1" 
                       or  $sqlJoin .= " join QUEUE_HOST on nodeId=hostId ";
...
"($sqlstatus) and ($sqlusers) and ($sqlsites) and ($sqlnodes) and ($sqlmasterjobs) and ($sqlids) order by $order $limit";
    ...
    $rresult = $self->{TASK_DB}->getFieldsFromQueueEx("*", "$sqlJoin where $where")
      or $self->{LOGGER}->error("CE", "In psdirect error getting data from database")
      and return;
  }
